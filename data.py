...
# Шаг 10, добавление цели "для программирование"
with open("goals.json", "r") as f:
    goals_add = json.load(f)
goals_add.update({"programming": "Для программирования"})

with open("goals.json", "w") as f:
    json.dump(goals_add, f)

with open("teachers.json", "r") as f:
    teachers_list = json.load(f)
teachers_id = [8, 9, 10, 11]
teachers_new_list = []
for teacher in teachers_list:
    if teacher["id"] in teachers_id:
        teacher["goals"].append("programming")
    teachers_new_list.append(teacher)
with open("teachers.json", "w") as f:
    json.dump(teachers_new_list, f)
